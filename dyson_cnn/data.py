...
def load_dataset(
    ...
    When ``paths`` contains a ``set_dataset_dir`` key (created by
    ``load_paths(set_name=...)``), dataset files are read from that
    per-set ``results/<set>/cnn/dataset/`` subdirectory. Otherwise falls
    back to ``project_dir``.
    ...
    """
    project_dir = Path(paths.get("set_dataset_dir", paths["project_dir"]))
